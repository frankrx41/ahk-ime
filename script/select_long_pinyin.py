...
cursor.execute("SELECT key FROM pinyin WHERE LENGTH(value) >= 5;")
# cursor.execute("SELECT key FROM pinyin WHERE LENGTH(value) >= 5 LIMIT 2;")

source_file = open(file_path, 'w')

print(
"; Store pinyin in db that LENGTH(value) >= 5\n; LENGTH(value) >= 5\n; Generate by " + os.path.basename(__file__) , file=source_file
)
