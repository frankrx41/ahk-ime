...
;                   provide an INI-File Yzime.ini in the script's folder containing the path:
;                   [Main]
...
Class SQLiteDB {
	...
	__New() {
		...
		If (This.Base._RefCount = 0) {
			...
			If !FileExist(SQLiteDLL)
				SQLiteDLL := This.Base._SQLiteDLL := DllFolder "\SQLite3.dll"
		...
		This._CBPtr := [], This._FCPtr := []
		This._CBPtr.gettable := RegisterCallback("callback_gettable", "F C", 4)
		This._FCPtr.exec := DllCall("GetProcAddress", "Ptr", DllCall("GetModuleHandle", "Str", "SQLite3", "Ptr"), "AStr", "sqlite3_exec", "Ptr")
	}
   ; ===================================================================================================================
   ; DESTRUCTOR __Delete
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
	LoadOrSaveDb(DBPath, isSave:=0){
		...
		If (DBPath = "")
			Return False
		If (!This._Handle) {
			This.ErrorMsg := "You must first Open Memory DB!"
			Return False
		}
		Flags := SQLITE_OPEN_READWRITE
		Flags |= SQLITE_OPEN_CREATE
		...
		pFrom := (isSave ? This._Handle : HDB)
    	pTo   := (isSave ? HDB     : This._Handle)
		This._StrToUTF8("main", UTF8)
		pBackup := DllCall("SQLite3.dll\sqlite3_backup_init", "Ptr", pTo, "Ptr", &UTF8, "Ptr", pFrom, "Ptr", &UTF8, "Cdecl Int")
		If (pBackup){
		...
		If (ErrorLevel) {
			This.ErrorMsg := "DLLCall sqlite3_backup failed!"
			This.ErrorCode := ErrorLevel
			If (isSave)
				FileDelete, %tmp%
			Return False
   ...
   ; ===================================================================================================================
   ; METHOD Exec           Execute SQL statement：执行SQL语句
   ; Parameters:           SQL         - Valid SQL statement：有效的SQL语句
   ...
   ; ===================================================================================================================
	Exec(SQL, Callback := "") {
		...
		RC := DllCall(This._FCPtr.exec, "Ptr", This._Handle, "Ptr", &UTF8, "Ptr", CBPtr, "Ptr", Object(This)
				  , "PtrP", Err, "Cdecl Int")
		...
		If (This._Path=":memory:"&&InStr(SQL, "'main'")&&(SQL~="i)^(UPDATE|INSERT|DROP|CREATE|DELETE)"))
			FileAppend, % RTrim(SQL,";") ";`n", %A_Temp%\Yzime\yzime_sql.tmp, UTF-8
		Return True
	}
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
	GetTable(SQL, ByRef TB) {
		...
		Err := 0, TB := [], TB.ColumnCount := 0, TB.RowCount := 0, TB.Rows := []
		This._StrToUTF8(SQL, UTF8)
		RC := DllCall(This._FCPtr.exec, "Ptr", This._Handle, "Ptr", &UTF8, "Ptr", This._CBPtr.gettable, "Ptr", Object(TB)
			, "PtrP", Err, "Cdecl Int")
		CallError := ErrorLevel
		If (CallError) {
			This.ErrorMsg := "DLLCall sqlite3_exec failed!"
...
SQLiteDB_RegExp(Context, ArgC, Values) {
	...
	If (ArgC = 2) {
		...
		; __Result := RegExMatch(StrGet(AddrH, "UTF-8"), StrGet(AddrN, "UTF-8"))
		regexNeedle:=DllCall("SQLite3.dll\sqlite3_value_text16", "Ptr", NumGet(Values + 0, "Ptr"), "Cdecl Str")
		search:=DllCall("SQLite3.dll\sqlite3_value_text16", "Ptr", NumGet(Values + A_PtrSize, "Ptr"), "Cdecl Str")
		__Result:=RegExMatch(search, regexNeedle)
	}
	DllCall("SQLite3.dll\sqlite3_result_int", "Ptr", Context, "Int", !!__Result, "Cdecl") ; 0 = false, 1 = trus
}
SQLiteDB_RegExpR(Context, ArgC, Values) {
	__Result := 0
	If (ArgC = 3) {
		search:=DllCall("SQLite3.dll\sqlite3_value_text16", "Ptr", NumGet(Values + 0, "Ptr"), "Cdecl Str")
		regexNeedle:=DllCall("SQLite3.dll\sqlite3_value_text16", "Ptr", NumGet(Values + A_PtrSize, "Ptr"), "Cdecl Str")
		replacestr:=DllCall("SQLite3.dll\sqlite3_value_text16", "Ptr", NumGet(Values + 2*A_PtrSize, "Ptr"), "Cdecl Str")
		__Result:=RegExReplace(search, regexNeedle, replacestr)
	}
	DllCall("SQLite3.dll\sqlite3_result_text16", "Ptr", Context, "Str", __Result, "Int", -1, "Ptr", 0, "Cdecl")
}
callback_gettable(TB, ColumnCount, ColumnValues, ColumnNames) {
	arr := [], TB := Object(TB)
	If !TB.HasKey("ColumnNames"){
		TB.ColumnCount := ColumnCount, TB.ColumnNames := []
		Loop %ColumnCount%
			TB.ColumnNames[A_Index] := StrGet(NumGet(ColumnNames + A_PtrSize * (A_Index - 1), "UInt"), "UTF-8")
	}
	Loop %ColumnCount%
		arr[A_Index] := StrGet(NumGet(ColumnValues + A_PtrSize * (A_Index - 1), "UInt"), "UTF-8")
	TB.Rows.Push(arr), TB.RowCount++
	Return 0
}
callback_DeriveTable(arg, ColumnCount, ColumnValues, ColumnNames){
	obj := Object(arg), buf:=""
	obj.file.Write(StrGet(NumGet(ColumnValues + 0, "UInt"), "UTF-8"))
	obj.RowCount++
}